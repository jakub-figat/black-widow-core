...
def test_game_service_create_lobby_and_add_player(game_service: GameService) -> None:
    ...
    game_service.user_data_access.save(model=user)

    lobby = game_service.create_lobby(user=user)
    game_service.add_user_to_lobby(lobby_id=lobby.lobby_id, user=other_user)

    lobby = game_service.lobby_data_access.get(**lobby.key)
    user = game_service.user_data_access.get(**user.key)
    other_user = game_service.user_data_access.get(**other_user.key)
...
def test_game_service_create_lobby_game_started_with_all_players(game_service: GameService) -> None:
    ...
    game_service.user_data_access.bulk_save(models=users)
    ...
    assert game_service.lobby_data_access.get(**lobby.key) is None

    users = [game_service.user_data_access.get(**user.key) for user in users]
    for user in users:
...
def test_game_service_remove_user_from_lobby(game_service: GameService) -> None:
    ...
    lobby = game_service.lobby_data_access.get(pk=lobby.pk, sk=lobby.sk)
    user = game_service.user_data_access.get(pk=user.pk, sk=user.sk)
...
def test_game_service_dispatch_game_action_raises_validation_error_with_invalid_payload(
    ...
    game_service.game_data_access.save(model=game)
